...
The `/room` (`CommitteeRoom`) is a single-column layout: a compact one-line **deal strip** (`DealBrief` — company · SSI · Reg · stage · sector + a `Full memo` button), a **Live ↔ Transcript** tab toggle, the committee conversation, and the presenter input below it. **Live view** (`CommitteeLive`) renders the member on the table now — their full question, large, directly above the input — plus a four-member roster; **Transcript view** (`TranscriptView`) is the full chronological log. Both render from the same `turns` array. The full memo opens in a right-anchored **drawer** (backdrop dimmer, Esc closes, body scroll lock) from the strip's `Full memo` button — that drawer is where `brief.sources` citations and the structured `enrichment` block render. The committee LLM still sees the full memo via `briefContext()` in `lib/committee.ts`.
...
The sentiment tag is **mandatory** for every committee reply — written by the LLM under `SHARED_RULES` in `lib/committee.ts`, parsed off the last line, and surfaced as a colored dot in `CommitteeLive`. If a model output omits it, `parseSentiment` silently defaults to `neutral`. The per-member sentiment trajectory is also surfaced in the post-room report (`ScoreReport` + `ReportPDF`) via `sentimentTrajectory()` in `lib/turn-router.ts`.
...
- **Motion**: framer-motion with house easing `[0.2, 0, 0, 1]` and durations 0.22-0.36s. Used in `CommitteeLive`, `DealBrief` drawer, `TimeoutWarning`, share-button toast, and the lookup panel. Re-use, don't invent.
